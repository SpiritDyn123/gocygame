...
func (mgr *svrsMgr) Start() (err error) {
	//注册消息
	global.LoginSvrGlobal.GetMsgDispatcher().Register(ProtoMsg.EmMsgId_SVR_MSG_REGISTER_LOGIN,
...
func (mgr *svrsMgr) OnAccept(tcp_session *tcp.Session) {
	logic_session := session.CreateClientSession(tcp_session, global.LoginSvrGlobal, &ProtoMsg.PbSvrBaseInfo{})
...
func (mgr *svrsMgr) onReqRegister(sink interface{}, head common.IMsgHead, msg proto.Message) {
	req_msg := msg.(*ProtoMsg.PbSvrRegisterLoginReqMsg)
